...
`::before` та `::after` не можна виділити чи скопіювати, їх немає в [replaced elements](https://developer.mozilla.org/en-US/docs/Web/CSS/Replaced_element) (`img`, `input`); вони додаються тільки коли є `content`, і стандартно мають `display: inline`.
...
<small>Різні методи письма може й не знадобляться для того,щоб працювати з різними мовами, але необхідні для розуміння логічних властивотей для розміру та полів 👇</small>
...
Приклад — генерація [типографічної шкали](https://spencermortensen.com/articles/typographic-scale/):
